...
# --- CentOS Linux 7.9.2009 Scan Section ---
SSH_KEY="${SSH_KEY_FROM_JENKINS:-/home/khushi.m/key.pem}"
REPORT_DEST="${SCRIPT_DIR}/openscap_reports"
mkdir -p "$REPORT_DEST"
...
# --- Amazon Linux 2023 Scan Section ---
SSH_KEY="${SSH_KEY_FROM_JENKINS:-/home/khushi.m/key.pem}"
REPORT_DEST="${SCRIPT_DIR}/openscap_reports"
mkdir -p "$REPORT_DEST"
...
REPORT_DEST="${SCRIPT_DIR}/openscap_reports"
mkdir -p "$REPORT_DEST"
FOLDER_NAME="customRockyLinux9"

echo ""
echo "[INFO] Starting OpenSCAP for Rocky Linux 9 instances..."
...
tail -n +2 "$FINAL_SSM_FILE" | while IFS=, read -r name ip platform version; do
    ...
    if [[ "$platform" == "Rocky Linux" && "$version" == "9" ]]; then
        ...
        if [[ -z "$FAIL_RULES" ]]; then
            ...
            for rule in $(echo "$FAIL_RULES" | cut -d':' -f1); do
                echo "[PATCH] Disabling rule: $rule"
                # Patching the tailoring file
                ssh -n -i "$SSH_KEY" "$SSH_USER@$ip" "sudo sed -i 's|\(<[^>]*idref=\"$rule\"[^>]*selected=\)\"true\"|\1\"false\"|' /tmp/$FOLDER_NAME/tailoring-xccdf.xml"
                # Patching the main datastream file
                ssh -n -i "$SSH_KEY" "$SSH_USER@$ip" "sudo sed -i 's|\(<[^>]*idref=\"$rule\"[^>]*selected=\)\"true\"|\1\"false\"|' /tmp/$FOLDER_NAME/ssg-rl9-ds.xml"
            done
